...
test('Validation of Report for search Specific transaction based on payment type Cash from the Pay Receipt Report', async ({ page }) => {
    ...
    await test.step('Verify the and transaction Details for the patient', async () => {
        const payment = await ezclaimpayreceiptpage.VerifySpecificTransactionDetailsByPaymentMethod("Andrew, Wagner","Cash");
        expect(payment).toEqual("CASH");
    });
})
